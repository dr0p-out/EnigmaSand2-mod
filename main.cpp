...
const char *resource_text[] = {
    "WALL",      "FIRE", "WATER", "PLANT",    "SAND",  "SALT",
    "WAX",       "OIL",  "SPOUT", "METHANE",  "NITRO", "NAPALM",
    "GUNPOWDER", "C-4",  "TORCH", "CONCRETE", "ERASER"};
int resource_color[] = {WALL, FIRE, WATER, PLANT, SAND,  SALT,
                        CERA, OIL,  SPOUT, METH,  NITRO, NAPA,
                        GUNP, C4,   TORCH, CONC,  BLANK};
const char *menu_text[] = {"PEN-size: x", "SAND: x",     "WATER: x",
                           "SALT: x",     "OIL: x",      "Time: x",
                           "Ground On: ", "XavierSoft: "};
...
const char *menu_val_text[][7] = {
    {"PEN-size: x1", "PEN-size: x2", "PEN-size: x4", "PEN-size: x8",
     "PEN-size: x16", "PEN-size: x24", "PEN-size: x32"},
    {"SAND: x0", "SAND: x1", "SAND: x2", "SAND: x3", "SAND: x4", "SAND: x5"},
    {"WATER: x0", "WATER: x1", "WATER: x2", "WATER: x3", "WATER: x4",
     "WATER: x5"},
...
int main(void) {
  ...
  if (SDL_Init(SDL_INIT_VIDEO) == -1) {
    fprintf(stderr, "Cannot init sdl video\n");
    return 1;
  }
  atexit(SDL_Quit);
...
#ifdef RESCON
  viewer =
      SDL_SetVideoMode(menu_values[9][menu_count[9]], sizheights[menu_count[9]],
                       32, SDL_HWSURFACE | SDL_DOUBLEBUF);

  screen = SDL_CreateRGBSurface(SDL_HWSURFACE, menu_values[8][menu_count[8]],
                                resheights[menu_count[8]], 32,
                                viewer->format->Rmask, viewer->format->Gmask,
                                viewer->format->Bmask, viewer->format->Amask);
  ...
  TTF_Init();
  if (!(font = TTF_OpenFont("tahoma.ttf", 16))) {
    fprintf(stderr, "Cannot open ttf font file\n");
    quitValue = 1;
    goto quit_no_font;
  }
  ...
  while (!quit) {
    ...
    eframe = SDL_GetTicks();
    if (capframe) {
      while (eframe - bframe < (1000 / targetrate)) {
        eframe = SDL_GetTicks();
      }
    }
  }
...
void drawMenu(void) {
  ...
  for (int i = 0; i < resource_n; i++) {
    // if(pen_type==resource_color[i]){noFill(); stroke(255);
    // rect((location/5)*80, mheight+(location%5)*16, 80, 16);}
    setColor(resource_color[i]);
    drawString(resource_text[i], (int)((location / 5) * 80),
               mheight + (location % 5) * 16);
    location++;
  }
  ...
  for (int i = 0; i < menu_n; i++) {
    setColor(-128);
    drawString(menu_val_text[i][menu_count[i]], xofset + (location / 5) * 100,
               mheight + (location % 5) * 16);
    location++;
  }

  if (overRect(0, mheight, (((float)resource_n) / 5 + 1) * 80, 80, mouseX,
               mouseY)) {
    int index = (mouseX / 80) * 5 + (mouseY - mheight) / 16;
    if (index < resource_n) {
      // if(index<resource_text.length && index>0){
      setColor(BLANK);
      fillRect((index / 5) * 80, mheight + (index % 5) * 16, 80, 16);
      if (mousePressed) {
        pen_type = index;
      }
      setColor(255);
      drawString(resource_text[index], (int)((index / 5) * 80),
                 mheight + (index % 5) * 16);
    }
  } else if (mousePressed && !pmousePressed) {

    int index = (int)(((mouseX - xofset) / 90) * 5) + (mouseY - mheight) / 16;
    if (overRect(xofset, mheight, (((float)menu_n) / 5 + 1) * 100, 80, mouseX,
                 mouseY)) {
      if (index < menu_n)
...
#endif
    // if(!checkresource&&overRect(xofset+(location/5)*90,
    // mheight+(location%5)*16, 90, 16,mouseX, mouseY)){
    // if(mousePressed&&!pmousePressed){menu_count[i]=(menu_count[i]+1)%(menu_values[i].length);}}
  }
  setColor(0xffffffff);
  drawRect((pen_type / 5) * 80, mheight + (pen_type % 5) * 16, 80, 16);
  setColor(resource_color[pen_type]);
  drawString(resource_text[pen_type], (int)((pen_type / 5) * 80),
             mheight + (pen_type % 5) * 16);
  drawingsurface = screen;
}
...
void pset(int i) {
  ...
  case (int)0xff347606: // nitro
    if (random(0.0F, 1.0F) < 0.3F && (touching(i, FIRE)) > -1) {
      explosion(i);
      return;
    }
    if (random(0.0F, 1.0F) < 0.2F && (touching(i, FIRE)) > -1) {
      pixels(i, FIRE);
    }
    if (fallingRule(i, true)) {
      return;
    }
    if (densityRule(i, OIL, 0.25F)) {
      return;
    }
    if (densityRule(i, WATER, 0.25F)) {
      return;
    }
    return;
  case (int)0xffeecc80: // sand
    ...
    if (densityRule(i, METH, 0.4F)) {
      return;
    }
    return;
  case (int)0xff2020ff: // water
    if (combineRule(i, SALT, SALTWATER, 0.25F)) {
      return;
    }
    if (densityRule(i, OIL, 0.25F)) {
      return;
  ...
  case (int)0xff804040: // oil
    if (random(0.0F, 1.0F) < 0.30F && (loc = touchingDirect(i, FIRE)) != -1) {
      doOilFire(i);
      return;
    }
    if (fallingRule(i, true)) {
  ...
  case (int)0xff20cc20: // plant
    if (growRule(i, WATER, 0.50F)) {
      return;
    }
    return;
  case -49088: // fire
    if (random(0.0F, 1.0F) < 0.2F && (loc = touching(i, PLANT)) != -1) {
      pixels(loc, FIRE);
      return;
    }
    if (random(0.0F, 1.0F) < 0.01F && (loc = touchingDirect(i, CERA)) != -1) {
      pixels(i, FIRE);
      pixels(loc, FIRE);
      if ((loc2 = below(max(i, loc), BLANK)) != -1) {
        pixels(loc2, CERA2);
      }
      return;
    }
    if (random(0.0F, 1.0F) < 0.4F &&
        (touching(i, PLANT) == -1 && touchingDirect(i, CERA) == -1)) {
      pixels(i, BLANK);
      return;
    }
    if (random(0.0F, 1.0F) < 0.8F && (loc = touching(i, WATER)) != -1) {
      pixels(loc, BLANK);
      pixels(i, BLANK);
      return;
    }
    if (random(0.0F, 1.0F) < 0.5F && (loc = above(i, BLANK)) > -1) {
      pixels(loc, FIRE);
    }
    return;
  case (int)0xffeeddcd: // falling cera
    if ((loc = belowD(i, BLANK)) != -1) {
      change(i, loc);
      return;
    } else {
      pixels(i, CERA);
  ...
  case (int)0xff8ba2a2: // concrete
    if (densityRule(i, WATER, 0.35F)) {
      return;
    }
    if (fallingRule3(i, true)) {
      return;
    } else {
      if ((belowD(i, WALL) > -1 || adjacent(i, WALL) > -1) &&
          random(0.0F, 1.0F) < 0.1F) {
        pixels(i, WALL);
      }
      if (random(0.0F, 1.0F) < 0.001) {
        pixels(i, WALL);
      }
    }
    return;
  case (int)0xffe8a21e: // napalm
    if (random(0.0F, 1.0F) < 0.25F && (touchingDirect(i, FIRE)) > -1) {
      doNapalm(i);
    }
    if (fallingRule(i, true)) {
      return;
  ...
  case (int)0xff9d866e: // gunpowder
    if (random(0.0F, 1.0F) < 0.95F && (touchingDirect(i, FIRE)) > -1) {
      doGunpowder(i);
      return;
    }
    if (fallingRule(i, true)) {
  ...
  case (int)0xfffaf29c: // C-4
    if (random(0.0F, 1.0F) < 0.6F && (touchingDirect(i, FIRE)) > -1) {
      doC4(i);
    }
    return;
...
bool combineRule(int i, int comcolor, int resultcolor, float chance) {
  int loc = 0;
  if (random(0.0F, 1.0F) < chance && ((loc = below(i, comcolor)) != -1 ||
                                      (loc = adjacent(i, comcolor)) != -1 ||
                                      (loc = above(i, comcolor)) != -1)) {
    pixels(i, BLANK);
    pixels(loc, resultcolor);
    return true;
  }
  return false;
}
...
bool growRule(int i, int comcolor, float chance) {
  int loc = 0;
  if (random(0.0F, 1.0F) < chance && ((loc = belowD(i, comcolor)) != -1 ||
                                      (loc = adjacent(i, comcolor)) != -1 ||
                                      (loc = aboveD(i, comcolor)) != -1)) {
    pixels(loc, pixels(i));
    return true;
  }
  return false;
...
bool risingRule(int i, bool adj) {
  ...
  if (random(0.0F, 1.0F) < 0.25F &&
      ((loc = aboveD(i, BLANK)) != -1 || (loc = adjacent(i, BLANK)) != -1)) {

    change(i, loc);
    return true;
  }
  // if( random(0.0F, 1.0F)<0.25F && (loc=adjacent(i, BLANK))!=-1){ change(i,
  // loc);return true;}
  return false;
}
...
bool fallingRule(int i, bool adj) {
  ...
  if (random(0.0F, 1.0F) < 0.95F &&
      ((loc = belowD(i, BLANK)) != -1 || (loc = adjacent(i, BLANK)) != -1)) {
...
bool fallingRule3(int i, bool adj) {
  ...
  if (random(0.0F, 1.0F) < 0.95F && belowD(i, CONC) != -1) {
    return true;
  }
  // if( random(0.0F, 1.0F)<0.25F && (loc=adjacent(i, BLANK))!=-1){ change(i,
  // loc);return true;}
...
void doOilFire2(int i, int avoid) {
  if (i - width > -1) {
    if (pixels(i - width) != avoid || random(0.0F, 1.0F) < 0.5F) {
      pixels(i - width, FIRE);
    }
  }
  if (pixels(i + width) != avoid || random(0.0F, 1.0F) < 0.5F) {
  ...
  if (pixels(i + 1) != avoid || random(0.0F, 1.0F) < 0.5F) {
    pixels(i + 1, FIRE);
  }
  if (pixels(i - 1) != avoid || random(0.0F, 1.0F) < 0.5F) {
    pixels(i - 1, FIRE);
...
void doSpigot(int i, int type, float rate) {
  if (random(0.0F, 1.0F) < rate) {
    if (i - width > -1 && pixels(i - width) == BLANK) {
      pixels(i - width, type);
    }
    if (pixels(i + width) == BLANK)
      pixels(i + width, type);
    if (pixels(i + 1) == BLANK)
      pixels(i + 1, type);
    if (pixels(i - 1) == BLANK)
...
int touchingDirect(int i, int type) {
  int loc = 0;
  if ((loc = adjacent(i, type)) != -1 || (loc = below(i, type)) != -1 ||
      (loc = above(i, type)) != -1) {
    return loc;
  }
  return -1;
...
int touching(int i, int type) {
  int loc = 0;
  if ((loc = adjacent(i, type)) != -1 || (loc = belowD(i, type)) != -1 ||
      (loc = aboveD(i, type)) != -1) {
    return loc;
  }
  return -1;
...
int adjacent(int i, int type) {
  int loc = 0;
  if (pixels(loc = (i - 1)) == type && pixels(loc = (i + 1)) == type) {
    return (random(0.0F, 1.0F) < 0.5F ? i - 1 : i + 1);
  }
  if (pixels(loc = (i - 1)) == type) {
    return loc;
  }
  if (pixels(loc = (i + 1)) == type) {
    return loc;
...
int below(int i, int type) {
  int loc = 0;
  if (i + width > sheight * width && type == BLANK) {
    return i + width;
  }
  if (pixels(loc = (i + width)) == type) {
    return loc;
...
int compoundVer(int i, int search) {
  ...
  int j = 0;
  while (pixels(i + j * width) == search) {
    count++;
    if (i + j * width < sheight * width) {
      break;
    }
    j++;
  }
...
void drawLine(int x1, int y1, int x2, int y2) { drawLine(x1, y1, x2, y2, 0); }
...
void fillCirc(int xpos, int ypos, int radius) {
  for (int x = ((xpos - radius - 1) < 0) ? 0 : (xpos - radius - 1);
       x <= xpos + radius && x < width; x++)
    for (int y = ((ypos - radius - 1) < 0) ? 0 : (ypos - radius - 1);
         y <= ypos + radius && y < height; y++) {
