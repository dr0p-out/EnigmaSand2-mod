...
void Particle::initiate(int i, int typ) {
  type = typ;
  xpos = i % width;
  ypos = i / width;
  active = true;
  ...
  case 1:
    strokecolor = FIRE;
    fillcolor = FIRE;
    psize = (int)(min(compoundHor(i, pixels(i)), compoundVer(i, pixels(i))) *
                  random(4.0F, 8.0F));
    xvel = 0;
    yvel = 0;
    break;
  ...
  case 3:
    ...
    if (count > 64) {
      count /= random(6.0F, 12.0F);
    } else {
      count *= random(1.0F, 3.0F);
    }
    psize = (int)(count);
    xvel = 0;
    yvel = 0;
